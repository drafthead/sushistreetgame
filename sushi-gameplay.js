(() => {
  ...
  proto.renderMenu=function(){if(U.menuItems)U.menuItems.innerHTML=''};
  proto.collectedCount=function(){return Object.values(this.menuCollected).reduce((a,n)=>a+Number(n||0),0)};
  ...
  proto.executeMove=function(row,col,lane){
    ...
    const x=this.colX(col),y=this.rowY(row);this.tweens.killTweensOf(this.player);this.tweens.killTweensOf(this.playerArt);
    this.tweens.add({targets:this.player,x,y,duration:128,ease:'Sine.InOut',onComplete:()=>{this.playerRow=row;this.playerCol=col;this.isMoving=false;this.player.y=y;if(lane.type==='water'){const f=this.findSupportAt(row,this.player.x);if(!f)return this.failRun('SPLASH DOWN','You jumped into the canal. Land on a lily pad, log, or fishing boat to cross.','water');this.playerSupport=f;this.playerSupportOffsetX=this.player.x-f.x}this.collectAt(row);this.updateMissedPickups();if(lane.type==='goal')return this.finishDelivery();if(this.bufferedMove){const n=this.bufferedMove;this.bufferedMove=null;this.requestMove(n.dx,n.dy)}}});
    this.tweens.add({targets:this.playerArt,y:-18,scaleX:.98,scaleY:1.03,duration:64,yoyo:true,ease:'Quad.Out',onComplete:()=>{if(this.playerArt)this.playerArt.setPosition(0,-7).setScale(1)}});this.tweens.add({targets:this.playerShadow,scaleX:.78,scaleY:.72,alpha:.58,duration:64,yoyo:true});
  };
